...
public class CalcGUI extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jLabel1.setText("—умма кредита:");
        jLabel2.setText("—рок кредита:");
        jLabel3.setText("мес€цев");
        jLabel4.setText("ѕроцентна€ ставка:");
        jLabel6.setText("%");
        clearall.setText("ќчистить всЄ");
        ...
        jLabel5.setText("≈жемес€чный платеж:    ");
        calculate.setText("–ассчитать");
        ...
        jLabel7.setText("—умма выплаты:");
        clearresult.setText("ќчист. рез.");
    ...
    double x; // коэффициент аннуитета (еж. платежа)
    double s; // сумма кредита
    double p; // проценты в мес€ц 
    double n; // количество мес€цев
    double ps; // проценты
    double ip; // 1+проценты в мес€ц
    double step; // 1+проценты в мес€ц в степени количества мес€цев
    double a; // размер аннуитета (ежемес€чного платежа)
    double t;
...
try 
{
    ...
    p=ps/100/12; // проценты в мес€ц 
    n=Double.parseDouble(String.valueOf(creditperiod.getSelectedItem()));
    ip= 1+p; // 1+проценты в мес€ц
    step=Math.pow(ip, n); // 1+проценты в мес€ц в степени количества мес€цев
    x=s*(p+(p/(step-1))); // коэффициент аннуитета (еж. платежа)
    a=Math.rint(100.0 * x)/100.0;  // размер аннуитета (ежемес€чного платежа)
    BigDecimal W = BigDecimal.valueOf(a);
    payment.setText(W.toString()); //ежемес€чный платеж
    t=Math.rint(100.0 * n)/100.0;
    BigDecimal S = BigDecimal.valueOf(a);
    BigDecimal q = BigDecimal.valueOf(n);
    BigDecimal Q = S.multiply(q); // —умма выплаты
